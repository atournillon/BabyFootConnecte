...
input_match_df = pd.read_sql_query("select a.* from PROD_LIVE_MATCH_HISTO as a INNER JOIN (SELECT distinct id_match FROM PROD_LIVE_MATCH_HISTO WHERE score_b = 10 OR score_r = 10) as b ON a.id_match = b.id_match;", cur)
...
match_results = fonction_statistiques.results_function(input_match_df[columns_results][(input_match_df.score_b == 10) | (input_match_df.score_r==10)].drop_duplicates())
